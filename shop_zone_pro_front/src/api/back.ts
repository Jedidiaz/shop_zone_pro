    ...
    if (error instanceof AxiosError) 
      if (error.response?.data.errors) {
        error.response?.data.errors.forEach(({ msg }: IErrors) => {
          ...
          if (error?.response?.status && [503, 401].includes(error?.response?.status)){
            redirect("/")
          }
        });
        return Promise.reject(error);
      }
      enqueueSnackbar(error.response?.data.msg || "Internal error, please reload page.", { autoHideDuration: 3000, variant: "error" });
      return Promise.reject(error);
  }
);
